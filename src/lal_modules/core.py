...
from . import pdfpage, pdfpainter
from .constants import (ADDR_COORDINATE, BOX_UPPDERLEFT_X_Y,
                        BOX_UPPDERRIGHT_X_Y, CC_RECEIVER_FIX_X_Y,
                        CHT_IN_RECT_X_Y, CONTENT_MAX_CHARACTER_PER_LINE,
                        CONTENT_MAX_LINE_PER_PAGE, CONTENT_X_Y_BEGIN,
                        CONTENT_X_Y_FIX, CONTENT_X_Y_INTERVAL, CUT_INFO_X_Y,
                        DEFAULT_FONT_PATH, DETAIL_START, DETAIL_Y_INTERVAL,
                        LETTER_FORMAT_PATH, LETTER_FORMAT_WIDE_HEIGHT,
                        NAME_COORDINATE, QUOTE_X_Y, RECT_X_Y_W_H, TITLE_START,
                        TITLE_Y_INTERVAL, VERSION)


def read_main_article(filepath):
    codec_name = 'utf-8'
    bom = b'\xef\xbb\xbf'.decode(codec_name)
    with open(filepath, 'r', encoding=codec_name) as f:
        text = f.read()
...
def generate_text_and_letter(senders, senders_addr,
                             ...
                             ccs, cc_addr,
                             main_text):
    text_path = gen_filename()
    letter_path = gen_filename()
    generator = pdfpainter.PDFPainter(text_path,
                                      LETTER_FORMAT_WIDE_HEIGHT[0], LETTER_FORMAT_WIDE_HEIGHT[1])
    blank_letter_producer = pdfpage.PDFPagePick(
        LETTER_FORMAT_PATH, letter_path)
    ...
    if one_page_is_enough:
        ...
        _fill_name_address_on_1st_page(
            generator, receivers, receivers_addr, 'r')
    ...
    if one_page_is_enough is False:
        _draw_info_box(generator, senders, senders_addr,
                       receivers, receivers_addr, ccs, cc_addr)
